...
model_decision_with_tier_clix <- read.csv("ModelDecisionMappingWithTierClix.csv", stringsAsFactors = F)
model_decision_with_tier_clix<- model_decision_with_tier_clix[!duplicated(model_decision_with_tier_clix[, c("ScoreBand", "BureauThickness", "Tier")]),]

model_decision_with_tier_blacksoil <- read.csv("ModelDecisionMappingWithTierBlacksoil.csv", stringsAsFactors = F)
model_decision_with_tier_blacksoil<- model_decision_with_tier_blacksoil[!duplicated(model_decision_with_tier_blacksoil[, c("ScoreBand", "BureauThickness", "Tier")]),]
...
p <- read.csv("Probability2.csv", stringsAsFactors = F)
...
source("variables.R")
source("Functions/1_Banking_Model.R")
...
Gen2 <- function(args){
  ...
  final_response$Model_Type <- list(name = "Model Type", value= "Level 1" )
  ...
  for (line_parameter_name in line_parameter_names){
    final_response[[line_parameter_name]] <- NA
  }
  ...
  ## Specific handling for tradeindia purchase financing
  tryCatch({
    if(grepl("tradeindia-purchasefinancing", application$anchor_name[1], ignore.case = T)){
      ...
      linefactor <- 0.7
      tradeindialine <- as.integer(applied_amount*linefactor)
      tradeindiadownpayment <- as.integer(applied_amount - tradeindialine)
      tradeindiatenure <- 7
      tradeindiapricing <- 0
      ...
      fast_loan_parameters$cibil_anchor_line_downpayment <- tradeindiadownpayment
  ...
  #### Set variable value against borrower for all rows on following columns

  business_variable_columns <- c("total_od_limit", "fast_loan_anchor_data", "fast_loan_anchor_name", "fast_loan_business_category", "vertical_risk_profile_band", "override_emi_obligation","instant_credit_approved_amount", "anchor_agent_type", "business_type", "merchant_id")
  ...
  application$is_key_promoter[is.na(application$is_key_promoter)] <- 0
  ...
  application$anchor_vintage2 <-  application$anchor_vintage
  ...
  #### Application details json converted to columns
    ...
    application$anchor_vintage[!is.na(as.numeric(application$anchor_vintage2)) & as.numeric(application$anchor_vintage2)>0] <-  application$anchor_vintage2[!is.na(as.numeric(application$anchor_vintage2)) & as.numeric(application$anchor_vintage2)>0]
    ...
    if(!is.na(data$created[1]) & data$created[1] >= as.Date("2018-02-15") ){
      model_decision_with_tier <- read.csv("ModelDecisionMappingWithTier_2018_02.csv", stringsAsFactors = F)
      model_decision_with_tier<- model_decision_with_tier[!duplicated(model_decision_with_tier[, c("ScoreBand", "BureauThickness", "Tier")]),]
    }
    ...
    model_decision_with_tier_row_clix <- model_decision_with_tier_clix[model_decision_with_tier_clix$ScoreBand == mean_band &
                                                                         model_decision_with_tier_clix$BureauThickness == data[ 1 , c("Bureau_thickness_gen2")] & 
                                                                         model_decision_with_tier_clix$Tier== location_tier, ]
    
    model_decision_with_tier_row_blacksoil <- model_decision_with_tier_blacksoil[model_decision_with_tier_blacksoil$ScoreBand == mean_band &
                                                                              model_decision_with_tier_blacksoil$BureauThickness == data[ 1 , c("Bureau_thickness_gen2")] & 
                                                                              model_decision_with_tier_blacksoil$Tier== location_tier, ]
    ...
    if(nrow(model_decision_with_tier_row_clix) == 1){
      clix_loan_cap <- model_decision_with_tier_row_clix$loan_cap
      clix_max_tenor <- model_decision_with_tier_row_clix$MaxTenor
      clix_pricing <- as.numeric(model_decision_with_tier_row_clix$pricing) + as.numeric(final$ExtraPricing)
      clix_model_decision <- model_decision_with_tier_row_clix$model_decision
    }else{
      clix_loan_cap <- NA
      clix_max_tenor <- NA
      clix_pricing <- NA
      clix_model_decision <- "Decline"
    }
    
    if(nrow(model_decision_with_tier_row_blacksoil) == 1){
      blacksoil_loan_cap <- model_decision_with_tier_row_blacksoil$loan_cap
      blacksoil_loan_tenure <- model_decision_with_tier_row_blacksoil$MaxTenor
      blacksoil_pricing <- as.numeric(model_decision_with_tier_row_blacksoil$pricing) + as.numeric(final$ExtraPricing)
      blacksoil_model_decision <- model_decision_with_tier_row_blacksoil$model_decision
    }else{
      blacksoil_loan_cap <- NA
      blacksoil_max_tenor <- NA
      blacksoil_pricing <- NA
      blacksoil_model_decision <- "Decline"
    }
    
    final$pre_pricing <- final$pricing
    ##################### Get the New Pricing ######################
    if(data$created[1] >= as.Date("2018-02-15") ){
      
    source("Functions/F1_InterestRate.R")
    tenure <- 12
    amount1 <- as.numeric(gsub("[^0-9.]","",data$amount[1] ))
    if(is.na(amount1)){  amount <- 0 }else{ amount <- amount1 }
    amount2 <- as.numeric(gsub("[^0-9.]","",data$loan_amount[1] ))
    if(!is.na(amount2)){ amount <- min(amount, amount2)  }
    
    if(tolower(data$Bureau_thickness_gen2)!="no hit" & !is.na(model_decision_with_tier_row$loan_cap)){
    amount <- min(as.numeric(amount), as.numeric(model_decision_with_tier_row$loan_cap))
    }
    
    final$pricing <- get_interest_rate( mean_band , 
          data[ 1 , c("Bureau_thickness_gen2")] , location_tier , 
           amount , 
          tenure ,   
          data[ 1 , c("industry")] )
    }
    ########### End of New Pricing Module ###########
    final2 <- final
    
    final$FinalDecision <- final$model_decision
    final2$FinalDecision[1] <- final$model_decision[1]
    
    #### Parameters which are same for all lenders
    
    final_response$tier <- final2$Tier
    final_response$tier_key <- list(name = "Tier" , value = final2$Tier )
    final_response$leverage <- list(name = "Leverage Ratio" , value =  paste0(round(as.numeric(data$leverage_ratio)*100,3),"%" ))
    ...
    final_response$pre_pricing <- final$pre_pricing
    ...
    #### Parameters which are not same for all lenders
    
    blacksoil_parameters <- as.list(final_response)
    
    final_response$model_decision$value = final2$FinalDecision[1]
    blacksoil_parameters$model_decision$value = blacksoil_model_decision
    ...
    if(blacksoil_model_decision != MODEL_DECISION_DECLINE){
      blacksoil_parameters$pricing$value = paste0(final2$pricing, "%")
      blacksoil_parameters$MaxTenor$value =blacksoil_loan_tenure
      blacksoil_parameters$loan_cap <-  blacksoil_loan_cap
    }
    ...
    if( final2$model_decision != MODEL_DECISION_APPROVE){
      ...
      final_response$model_reasons[length(final_response$model_reasons)+1] <- "Application is from Tier 4 location"
      }
    }
    if( blacksoil_model_decision != MODEL_DECISION_APPROVE){
      blacksoil_parameters$model_reasons <- reason_bad
      blacksoil_parameters$model_reasons <- blacksoil_parameters$model_reasons[[1]]
      if(blacksoil_parameters$tier == "Tier4" | blacksoil_parameters$tier == "Tier3"){
        blacksoil_parameters$model_reasons[length(blacksoil_parameters$model_reasons)+1] <- "Application is from Tier 3 or Tier 4 location"
      }
    }

    if(final2$FinalDecision[1] != MODEL_DECISION_DECLINE){
      final_response$notes <- c(final_response$notes, "Increase Pricing by 1% for Loan tenure of 18 Months & 2% for 24 Months or above")
      final_response$notes <- c(final_response$notes, "Price subject to a floor of 16% pricing")
    }
    if(blacksoil_model_decision != MODEL_DECISION_DECLINE){
      blacksoil_parameters$notes <- c(blacksoil_parameters$notes, "Increase Pricing by 1% for Loan tenure of 18 Months & 2% for 24 Months or above")
      blacksoil_parameters$notes <- c(blacksoil_parameters$notes, "Price subject to a floor of 16% pricing")
    }
    ...
    clix_parameters <- as.list(final_response)
    ############### Line Assignment ###################
    ...
    #### Horizontal Line Assignment - Horizontal, Vertical, CIBIL, CIBIL Anchor
    tryCatch({
      if(final2$model_decision != MODEL_DECISION_DECLINE){
      ...
      fast_loan_parameters$cibil_anchor_line_decision <- cibil_anchor_line_result[[7]]
      
      if(!is.na(cibil_anchor_line_result[[7]])){
        fast_loan_parameters$model_decision <- cibil_anchor_line_result[[7]]
      }
      ...
      existing_EMI_obligation2 <- round(existing_EMI_obligation)
      existing_EMI_obligation2[is.na(existing_EMI_obligation2)] <- "Not available"
      existing_EMI_obligation2[existing_EMI_obligation2!="Not available"] <- paste("Rs.",existing_EMI_obligation2[existing_EMI_obligation2!="Not available"])
      ...
      line_result <- c(horizontal_line_result[[1]], 
                       ...
                       cibil_anchor_line_result[[8]],
                       existing_EMI_obligation2)
      line_parameter_names <- c("Horizontal Line",
                                "Vertical Line", "Eligible for OD", "Vertical Max Tenor",
                                "CIBIL Line",
                                "CIBIL Anchor Line", "CIBIL Anchor Tenure", "Cibil Anchor Pricing", "Avg Anchor Monthly Txns",
                                "Existing EMI obligation")
      ...
      if(is.null(final_response$statements)){final_response$statements <- c()}
    ...
    tryCatch({
      if(final2$model_decision != MODEL_DECISION_DECLINE){
        ...
        horizontal_line_result_clix <- get_horizontal_line(data,
                                                           existing_EMI_obligation,
                                                           pricing,
                                                           maxTenor,
                                                           clix_loan_cap,
        ...
        horizontal_line_parameters_clix <- horizontal_line_result_clix[[3]]
        horizontal_line_parameters_clix$total_od_limit <- data$total_od_limit
        
        line_reasons_clix <- c(paste("Horizontal Line Reason :" , horizontal_line_result_clix[[2]]))
        
        if(!(is.na(horizontal_line_result_clix[[1]]) | horizontal_line_result_clix[[1]]=="")){
          clix_parameters[["Horizontal Line"]] <- list(name="Horizontal Line", value=horizontal_line_result_clix[[1]])
        }
        if(is.null(clix_parameters$statements)){
          clix_parameters$statements <- c()
        }
        
        clix_parameters$horizontal_line_parameters <- horizontal_line_parameters_clix
        
        clix_parameters$statements <- c(clix_parameters$statements, line_reasons_clix)
        
      }
    }, error=function(e){})
    
    #### Blacksoil Line Assignment
    tryCatch({
      if(blacksoil_model_decision != MODEL_DECISION_DECLINE){
        
        pricing <- final2$pricing
        maxTenor <- as.numeric(gsub("[^0-9.-]","",blacksoil_loan_tenure))
        
        horizontal_line_result_blacksoil <- get_horizontal_line(data,
                                                                existing_EMI_obligation,
                                                                pricing,
                                                                maxTenor,
                                                                blacksoil_loan_cap,
        ...
        horizontal_line_parameters_blacksoil <- horizontal_line_result_blacksoil[[3]]
        horizontal_line_parameters_blacksoil$total_od_limit <- data$total_od_limit
        
        line_reasons_blacksoil <- c(paste("Horizontal Line Reason :" , horizontal_line_result_blacksoil[[2]]))
        
        if(!(is.na(horizontal_line_result_blacksoil[[1]]) | horizontal_line_result_blacksoil[[1]]=="")){
          blacksoil_parameters[["Horizontal Line"]] <- list(name="Horizontal Line", value=horizontal_line_result_blacksoil[[1]])
        }
        if(is.null(blacksoil_parameters$statements)){
          blacksoil_parameters$statements <- c()
        }
        
        blacksoil_parameters$horizontal_line_parameters <- horizontal_line_parameters_blacksoil
        
        blacksoil_parameters$statements <- c(blacksoil_parameters$statements, line_reasons_blacksoil)
    ...
    tryCatch({
      ...
      if(is.null(final_response$notes)){final_response$notes <- c()}
      ...
      fast_loan_qualification_criteria_result_list <- get_qualification_criteria(data,
                                                                                industry_details[1],
                                                                                final_response[["Vertical Line"]]$value,
                                                                                "fast_loan")
      ...
      clix_qualification_criteria_result_list <- get_qualification_criteria(data,
                                                                       industry_details[1],
                                                                       final_response[["Vertical Line"]]$value,
                                                                       "clix")
      
      clix_bad_criteria_count <- length(clix_qualification_criteria_result_list$v1)
      clix_qualification_criteria_result_list$v1 <- clix_qualification_criteria_result_list$v1[clix_qualification_criteria_result_list$v1 != ""]
      clix_qualification_criteria_result <- c(clix_qualification_criteria_result_list$v1, clix_qualification_criteria_result_list$v2)
      
      final_response$qualifying_criteria <- list()
      clix_parameters$qualifying_criteria <- list()
      blacksoil_parameters$qualifying_criteria <- list()
      
      if(length(qualification_criteria_result)==1){
        final_response$qualifying_criteria <- list(qualification_criteria_result)
        blacksoil_parameters$qualifying_criteria<- list(qualification_criteria_result)
      }else if(length(qualification_criteria_result)>0 & !is.null(qualification_criteria_result)){
        final_response$qualifying_criteria <- qualification_criteria_result
        blacksoil_parameters$qualifying_criteria<- qualification_criteria_result
      }
      
      if(length(clix_qualification_criteria_result)==1){
        clix_parameters$qualifying_criteria <- list(clix_qualification_criteria_result)
      }else if(length(clix_qualification_criteria_result) >0 & !is.null(clix_qualification_criteria_result)){
        clix_parameters$qualifying_criteria <- clix_qualification_criteria_result
      }
      ...
      if(bad_criteria_count > 0 & final_response$model_decision$value == MODEL_DECISION_APPROVE){
        ...
        if(fast_loan_parameters$model_decision == MODEL_DECISION_APPROVE){
          if(!is.null(fast_loan_parameters$cibil_anchor_line_amount)){
            if(!is.na(fast_loan_parameters$cibil_anchor_line_amount)){
      ...
      #### Blacksoil criteria
      if(bad_criteria_count > 0 & blacksoil_parameters$model_decision$value == MODEL_DECISION_APPROVE){
        blacksoil_parameters$model_decision$value <- MODEL_DECISION_MANUAL_REVIEW
        blacksoil_parameters$model_reasons <- list(paste(qualification_criteria_result_list$v1, collapse = " , "))
      }
      
      #### Clix criteria
      if(clix_bad_criteria_count > 0 & clix_parameters$model_decision$value == MODEL_DECISION_APPROVE){
        clix_parameters$model_decision$value <- MODEL_DECISION_MANUAL_REVIEW
        clix_parameters$model_reasons <- list(paste(clix_qualification_criteria_result_list$v1, collapse = " , "))
      }
      
      if(fast_loan_bad_criteria_count > 0 & fast_loan_parameters$model_decision == MODEL_DECISION_APPROVE){
        if(is.null(fast_loan_parameters$cibil_anchor_line_amount) | is.na(fast_loan_parameters$cibil_anchor_line_amount)){
          fast_loan_parameters$model_decision <- MODEL_DECISION_MANUAL_REVIEW
        }
      }
    ...
    ## Change model decision to Manual Reviews in case important variables are missing - ie pincode and vintage
    
    variablesMissing <- F
    if(is.null(data$pin) | is.null(data$vintage)){variablesMissing <- T}
    if(is.na(data$pin) | is.na(data$vintage) | data$vintage==0){variablesMissing <- T}
    ...
    if(variablesMissing){
      ...
      blacksoil_parameters$model_decision$value <- MODEL_DECISION_MANUAL_REVIEW
      fast_loan_parameters$model_decision <- MODEL_DECISION_MANUAL_REVIEW
      final_response$statements <- c(final_response$statements, "Pincode or incorporation date is not filled. Please fill data.")
    }
    
    ## Nested json added to final response
    
    final_response$clix_parameters <- clix_parameters
    final_response$blacksoil_parameters <- blacksoil_parameters
    
    final_response$fast_loan_parameters <- fast_loan_parameters
    final_response$instant_credit_parameters <- instant_credit_parameters
    
    # ###########  Fast Loan Horizontal Line ###########
    # if(!is.na(as.numeric(final_response$`Horizontal Line`$value)) ){
    #   if(as.numeric(final_response$`Horizontal Line`$value) > 25000 ){
    #     if(is.na(as.numeric(final_response$fast_loan_parameters$fast_loan_line))){
    #       final_response$fast_loan_parameters$fast_loan_line <- as.numeric(final_response$`Horizontal Line`$value)
    #       final_response$fast_loan_parameters$fast_loan_line <- "null"
    #     }else{
    #       if(as.numeric(final_response$fast_loan_parameters$fast_loan_line) < as.numeric(final_response$`Horizontal Line`$value) )
    #         final_response$fast_loan_parameters$fast_loan_line <- as.numeric(final_response$`Horizontal Line`$value)
    #       final_response$fast_loan_parameters$fast_loan_line <- "null"
    #     }
    #     
    ...
    ########### Behold the Banking Model is Starting ############
    
    #######  Default Probability ####
    tryCatch({
    probability <- 0.11
    normalized_score <- 0.11
    if(data$Bureau_thickness_gen2 == "Thick"){
    normalized_score <- (1-(data$ExperianScore -460)/235)*0.4 + data$Gen2Score*0.6
    probability <- p$value[p$variable=="intercept"] + p$value[p$variable=="normalized_score"]*normalized_score
    probability <- 1/(1+exp(-1*probability))
    }
    if(data$Bureau_thickness_gen2 == "Thin"){
      normalized_score <- (1-(data$ExperianScore -460)/235)*0.15 + data$Gen2Score*0.85
      probability <- p$value[p$variable=="intercept"] + p$value[p$variable=="normalized_score"]*normalized_score
      probability <- 1/(1+exp(-1*probability))
    }
    
    is_od <- as.numeric(data$is_od)
    avg_eod <- as.numeric(data$avg_eod)
    eod_l3 <- as.numeric(data$eod_l3)
    eod_l3_w <- as.numeric(data$eod_l3_w)
    avg_credits <- as.numeric(data$avg_credits)
    credits_l3 <- as.numeric(data$credits_l3)
    credits_macro_growth <- as.numeric(data$credits_macro_growth)
    credits_micro_growth <- as.numeric(data$credits_micro_growth)
    bureau_thickness <- data$Bureau_thickness_gen2
    stability_eod <- as.numeric(data$stability_eod)
    stability_credits <- as.numeric(data$stability_credits)
    applied_amt <- as.numeric(gsub("[^0-9.]","",data$amount))
    inward_cheque_bounces <- as.numeric(data$inward_cheque_bounces_12m)
    HS_decision <- final$model_decision
    HS_probability <- as.numeric(probability)
    eod_l3_w_non_od_N <- as.numeric(data$eod_l3_w_non_od_n)
    penal_transactions <- as.numeric(data$penal_transactions)
    delinquency_index_lifetime <- as.numeric(data$delinquency_index_lifetime)
    leverage_ratio <- as.numeric(data$leverage_ratio)
    business_vintage <- as.numeric(data$business_vintage_in_years)
    Age <- as.numeric(data$Age)
    cibil_score <-  as.numeric(data$cibil_score)
    owned_all <-  as.numeric(data$owned_all)
    
    bs_output <- bs_model(is_od , avg_eod , eod_l3 , eod_l3_w , eod_l3_w_non_od_N , avg_credits , credits_l3 , credits_macro_growth , credits_micro_growth , bureau_thickness , stability_eod , stability_credits , applied_amt , penal_transactions , HS_decision , HS_probability  , delinquency_index_lifetime , leverage_ratio , business_vintage , inward_cheque_bounces , owned_all , Age , cibil_score)
    if(!is.na(bs_output$reason)){
    notes_bs <- fromJSON(bs_output$reason)
    final_response$model_reasons <- c(final_response$model_reasons, notes_bs)
    }
    bs_output$reason <- NULL
    final_response$default_probability <- as.numeric(bs_output$bs_model_score)*10/13
    final_response$bs_vars <- bs_output
    
    if(!is.na(final_response$default_probability)){
    final_response$score <- as.numeric(bs_output$bs_model_score)*10/13
    final_response$HS_Band <- final_response$score_band$value
    final_response$score_band$value <- bs_output$bs_rb 
    final_response$model_decision$value <- bs_output$bs_decision 
    final_response$Model_Type <- list(name = "Model Type", value= "Level 2" )
    final_response$bank_statement_required$value  <- " Bank Statement is Present"
    final_response$notes <- c(final_response$notes, paste0("Horizontal Risk band is ", final_response$HS_Band, " and Horizontal Decision is ", HS_decision))
    }
    }, error = function(e){})
